...
export function superLob(game, plant) {
    ...
    if (!z) return;
    throwAt(game, plant, z);
    game.sound.playShoot();
}

// 抛一发到某只僵尸身上，砸中后接着弹下一跳，一直弹到这一排空了为止
function throwAt(game, plant, zombie, from = null) {
    lob(game, plant, { zombie }, 0, {
        cls: 'lob-tri', html: TRI_HTML, size: SIZE, from,
        damage: SUPER_DAMAGE, splashCls: 'tri-splash', onHit: butterUp,
        after: (g, t, c) => {
            const rowY = t.zombie.y;
            const next = nextInRow(g, rowY, c.x, t.zombie);
            if (next) throwAt(g, plant, next, { x: c.x - SIZE / 2, y: c.y - SIZE / 2 });
...
export function superVolley(game, plant) {
    ...
    targets.forEach((t, i) => lob(game, plant, t, Math.min(i * STAGGER_MS, MAX_STAGGER), {
        ...
        damage: SUPER_ULT_DAMAGE, splashCls: 'tri-splash big', onHit: butterUp,
    }));
