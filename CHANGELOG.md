...
### 子弹伤害递增
- 每株植物打出的每一颗子弹，伤害比它上一颗高 15%，复利叠加，封顶 6700 万
  （`CombatManager` 的 `DAMAGE_GROWTH` / `DAMAGE_CAP`）
- 基础伤害 17000 起步，同一株第 61 发触顶（17000 × 1.15^60 ≈ 6805 万，被削到 6700 万）
...
- 去掉开局第一列自动种的 5 株随机植物：池子里的超级投手 / 胶水 / 黑曜石都是
  ...
  自动通关。现在空场开局，输出得自己种
- 博士血量 64000 → 6400 万
- 所有弹种统一 17000 伤害（`BULLET_DAMAGE`）：豌豆 / 寒冰 / 原始豌豆 / 卷心菜 /
  机枪豌豆 / 散射豌豆原来是 20~70，加特林 / 胶水 / 黑曜石 / 超级投手 / 水滴 /
  保龄球原来是 Infinity「秒杀哨兵」，现在一律 17000。僵尸满血 800，对僵尸
  仍是一发一个；对博士就是实打实 17000 一发（不算递增要 3765 发；算上逐发 +15%
  的递增，同一株约 46 发就能打满 6400 万）。
  `BOSS_SENTINEL_DAMAGE` 折算只留作兜底，已经用不上了
